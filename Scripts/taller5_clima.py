...
# BASE_DIR = carpeta que contiene este script, subiendo un nivel (la raíz
# del proyecto). Así las rutas funcionan sin importar cuál sea el working
# directory activo en Spyder.
...
# --- 3.2 CO2 (NOAA Mauna Loa, archivo Excel) -------------------------------
# Este archivo ya viene limpio, con columnas: Year, Month, Monthly average,
# Interpolated, Trend. "Monthly average" trae -99.99 cuando falta el dato
# ese mes; "Interpolated" ya rellena esos huecos; "Trend" es la serie
# desestacionalizada (sin el ciclo anual).
co2 = pd.read_excel(ARCHIVO_CO2)
assert {"Year", "Month", "Interpolated", "Trend"}.issubset(co2.columns), (
    f"Columnas encontradas: {list(co2.columns)}. Revisa si cambiaron los "
    "nombres en el archivo."
)
co2["Monthly average"] = co2["Monthly average"].replace(-99.99, np.nan)
co2 = co2.rename(columns={
    "Year": "year", "Month": "month",
    "Interpolated": "co2_ppm", "Trend": "tendencia",
})
...
# "co2_ppm" = serie interpolada (rellena huecos mensuales); "tendencia" =
# serie desestacionalizada (sin el ciclo anual de fotosíntesis/respiración
# de la vegetación del hemisferio norte).

# --- 1.3.3: línea de CO2 desde 1960 ---------------------------------------
co2_recent = co2[co2["year"] >= 1960].copy()
co2_recent["fecha"] = pd.to_datetime(dict(year=co2_recent["year"], month=co2_recent["month"], day=1))

fig, ax = plt.subplots()
ax.plot(co2_recent["fecha"], co2_recent["co2_ppm"], label="Interpolated", linewidth=1)
ax.plot(co2_recent["fecha"], co2_recent["tendencia"], label="Trend", linewidth=1.5)
...
co2_mes = co2.loc[co2["month"] == MES_NUM, ["year", "co2_ppm"]].rename(columns={"year": "Year"})
temp_co2 = pd.merge(temp.reset_index()[["Year", MES]], co2_mes, on="Year", how="inner").dropna()
assert len(temp_co2) > 0, "El merge no produjo filas; revisa el rango de años en ambos archivos."

fig, ax = plt.subplots()
ax.scatter(temp_co2[MES], temp_co2["co2_ppm"], color="black", s=25)
ax.set_title("Anomalía de temperatura vs. CO2")
ax.set_xlabel(f"Anomalía de temperatura en {MES} (°C)")
ax.set_ylabel("CO2 (ppm)")
guardar(fig, "1_3_4_dispersión.png")

r, p_valor = stats.pearsonr(temp_co2[MES], temp_co2["co2_ppm"])
...
print(f"\nListo. Gráficos en {os.path.join(CARPETA_SALIDA, 'Figuras')}, tabla en "
      f"{os.path.join(CARPETA_SALIDA, '1_2_5_media_varianza.csv')}")
